...
startbutton.addEventListener("click", function() {
    ...
    function spawnSeed() {
        ...
        seed.draggable = true;
        seed.id = "seed-" + seedCounter++; // Eindeutige ID
    ...
    function spawnSeedling(x, y) {
        ...
        seedling.id = "seedling-" + seedlingCounter++; // Eindeutige ID
    ...
    function spawnFlower(x, y) {
        ...
        flower.id = "flower-" + flowerCounter++;
        flower.dataset.type = "flower"; // Eindeutige ID
    ...
    function spawnMegaFlower(x, y, element) {
        ...
        megaFlower.id = "megaFlower-" + megaFlowerCounter++; // Eindeutige ID
    ...
    function addDragAndDrop(element) {
        ...
        document.getElementById("dropArea").addEventListener("drop", event => {
            ...
            if (draggedItem) {
                ...
                let parentOffset = getTotalOffset(event.currentTarget);
    ...
    function checkForCombination(movedSeed) {
        ...
        seeds.forEach(seed => {
            if (seed !== movedSeed && seed.dataset.type === "seed" && isOverlappingSeed(seed, movedSeed)) {
                ...
                // Entferne beide Samen
                seed.remove();
                movedSeed.remove();

                // Erstelle Seedling genau in der Mitte der beiden Samen
                spawnSeedling(seedX, seedY);
            }
        });
    }

    // Prüft, ob sich 2 Seedlings überlappen
    function checkForCombination2(movedSeedling) {
        ...
        seedlings.forEach(seedling => {
            if (seedling !== movedSeedling && seedling.dataset.type === "seedling" && isOverlappingSeedling(seedling, movedSeedling)) {
                ...
                // Entferne beide Seedlings
                seedling.remove();
                movedSeedling.remove();

                // Erstelle Flower genau in der Mitte der beiden Seedlings
                spawnFlower(seedlingX, seedlingY);
            }
        });
    }

    // Prüft, ob sich 2 Blumen überlappen
    ...
    // Alle 5 Sekunden einen Samen spawnen 
